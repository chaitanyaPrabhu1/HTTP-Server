...
function cleanBody(body) {
    ...
    const cleaned = words.map((word) => profane.has(word.toLowerCase()) ? "****" : word);
    return cleaned.join(" ");
}
...
async function handlerChirpsCreate(req, res, next) {
    try {
        const param = req.body;
...
async function handlerChirpGet(req, res, next) {
    try {
...
async function handlerUserUpdate(req, res, next) {
    try {
        const token = getBearerToken(req);
        const userId = validateJWT(token, config.jwt.secret);
        const param = req.body;
...
async function handlerChirpsDelete(req, res, next) {
    try {
        const token = getBearerToken(req);
        const userId = validateJWT(token, config.jwt.secret);
        const chirpId = req.params.chirpId;
...
async function handlerChirpsGet(req, res, next) {
    try {
